...
RSpec.describe 'Fighter' do
  context 'index performance' do
    it 'should be fast' do
      expect(Benchmark.realtime { get 'http://localhost:3000/api/v1/fighters' }).to be < 0
    end
  end
  ...
  context 'show performance' do
    it 'should be fast' do
      expect(Benchmark.realtime { get 'http://localhost:3000/api/v1/fighter/1' }).to be < 0
    end
  end
